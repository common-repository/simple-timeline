...
Version: 0.9
Plugin URI: http://wordpress.org/plugins/simple-timeline/
Author: Kuma Xu
...
function STL_init() {
	load_plugin_textdomain( 'Simple-Timeline', false, dirname( plugin_basename( __FILE__ ) ) .'/lang/' );
}
...
function STL_show_timeline() {
	...
	if($nodes){
		...
		$output .= '<span class="alert">'.__( 'Sorry, There is not any timeline node. Please create ata least one.', 'Simple-Timeline' ).'</span>';
	}
	$output .='</ul>';
	return $output;
}
